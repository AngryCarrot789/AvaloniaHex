...
public sealed class Caret {
    ...
    private void OnModeChanged() {
        this.ModeChanged?.Invoke(this, EventArgs.Empty);
    }
    ...
    public void GoToStartOfLine() {
        ...
        ulong byteIndex = (this.Location.ByteIndex / bytesPerLine) * bytesPerLine;
    ...
    public void GoToEndOfLine() {
        ...
        ulong bytesPerLine = (ulong) this.HexView.ActualBytesPerLine;
        ulong byteIndex = Math.Min(((this.Location.ByteIndex / bytesPerLine) + 1) * bytesPerLine, this.HexView.Document.Length) - 1;
    ...
    public void GoBackward(ulong byteCount) {
        ...
        this.Location = this.Location.ByteIndex >= byteCount
            ? new BitLocation(this.Location.ByteIndex - byteCount, this.Location.BitIndex)
            : new BitLocation(0, this.PrimaryColumn.FirstBitIndex);
    }
    ...
    public void GoForward(ulong byteCount) {
        ...
        if (document.Length < byteCount
            || this.Location.ByteIndex >= document.Length - byteCount) {
            this.Location = new BitLocation(document.Length, this.PrimaryColumn.FirstBitIndex);
            return;
        }
