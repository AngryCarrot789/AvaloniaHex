...
public class HexView : Control, ILogicalScrollable {
    /// <summary>Dependency property for <see cref="Document"/>.</summary>
    ...
    public static readonly StyledProperty<Thickness> HeaderPaddingProperty = AvaloniaProperty.Register<HexView, Thickness>(nameof(HeaderPadding));

    /// <summary>Dependency property for <see cref="IsHeaderVisible"/>.</summary>
    public static readonly StyledProperty<bool> IsHeaderVisibleProperty = AvaloniaProperty.Register<HexView, bool>(nameof(IsHeaderVisible), true);
    ...
    public void InvalidateVisualLine(BitLocation location) {
        var line = this.GetVisualLineByLocation(location);
        if (line != null)
    ...
    public void InvalidateVisualLines(BitRange range) {
        ...
        foreach (var line in this.GetVisualLinesByRange(range))
            line.Invalidate();
    ...
    protected override Size ArrangeOverride(Size finalSize) {
        ...
        if (this.Document is { } document) {
            this.Extent = new Size(0, Math.Ceiling((double) document.Length / this.ActualBytesPerLine));
        }
        else {
            this.Extent = default;
        }
    ...
    private void EnsureHeaders() {
        ...
        for (int i = 0; i < this.Columns.Count; i++) {
            ...
            var headerLine = this.Headers[i] ??= column.CreateHeaderLine();
            if (headerLine != null)
    ...
    private void UpdateVisualLines(Size finalSize) {
        ...
        if (this.Document.Length == 0) {
            ...
            var line = this._visualLines.GetOrCreateVisualLine(new BitRange(0, 1));

            line.EnsureIsValid();
            line.Bounds = new Rect(0, this.EffectiveHeaderSize, finalSize.Width, line.GetRequiredHeight());
        ...
        var enclosingRange = this.Document.ValidRanges.EnclosingRange;
        ...
        while (currentY < finalSize.Height && currentRange.End <= enclosingRange.End) {
            ...
            var line = this._visualLines.GetOrCreateVisualLine(new BitRange(
    ...
    public bool BringIntoView(BitLocation location) {
        if (this.Document is not { ValidRanges.EnclosingRange: var enclosingRange }
